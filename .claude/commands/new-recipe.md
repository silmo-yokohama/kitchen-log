...
4. `recipes/*.json` をGrepし、食材名の表記を既存のものに合わせる。カテゴリー・素材・タグは `masters/taxonomy.json` と照合し、無い値は先にマスタへ追加する
...
7. `docs/workflows/nutrition-rules.md` の計算式で栄養価（拡張栄養素5項目を含む）を算出し、`benefits`/`cautions`/`balance`/`summary`（総評）を記述する
