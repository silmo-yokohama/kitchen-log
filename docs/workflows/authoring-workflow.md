...
2. **url**: レシピサイトのURL。WebFetchで取得できる範囲の情報を元にする。YouTube等の動画プラットフォームは概要欄・字幕がWebFetchで取得できないため対象外とし、動画ベースのレシピは最初からテキスト貼り付け（type: text）に切り替えるようユーザーに伝える（ログイン必須のInstagram等も同様に対象外）
